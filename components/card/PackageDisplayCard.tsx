...
const PackageDisplayCard = React.memo(({ pkg }: PackageDisplayCardProps) => {
    ...
    return (
        <Link href={`/package/${pkg._id}`} className={"cursor-pointer"}>
            <div className="w-full border-2 rounded-2xl p-4 hover:scale-95 relative shadow-lg hover:shadow-xl transition-all duration-300 bg-white group">
                {/* Featured Badge */}
                {pkg.isFeatured && (
